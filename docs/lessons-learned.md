...
Across all three of this session's A/B trials, arms inspected the `--plugin-dir` path the harness passed them (`C:\sf-*-plugins\armN\`) and treated it as part of the project under review. The worst case built a *blocking* finding on it: an F1 control arm reported the harness copy as "a third lineage of the skills," noted its `plugin.json` already read the version the spec proposed bumping to, and asked the dispatcher to resolve the divergence before proceeding. Another arm disclosed that its loaded skill cited a pattern file absent from the fixture's own `docs/patterns/`. The contamination stayed symmetric across arms — both arms of every pair could see their own harness copy — so no comparison was biased, but a false blocking finding consumes the attention a real finding needs, and an asymmetric variant (arms differing in what their harness copy reveals) would corrupt a verdict silently. **Rule:** treat the `--plugin-dir` directory as part of the trial's fixture surface — before running, check what the harness copy itself reveals (version fields, edited skill text, files the fixture lacks), keep it identical across arms except for the line under test, and read any arm finding that cites the harness path as a trial artifact, not as a defect in the fixture's project.

**Tags:** none yet — tags deferred.

*Pattern promoted — folded into docs/patterns/seed-trial-fixtures-with-real-docs.md as a third rule. Compared against all 28 existing Rule sentences per the promotion step's enumeration: two name the same act — "A --plugin-dir trial fixture needs the real convention docs it's testing copied in" and "A trial fixture's git history leaks what its working tree no longer does." All three gather a trial's output while an unexamined channel (missing doc, git history, harness directory) shapes what the agent sees; both prior entries already promoted into that pattern, so this joins them rather than opening a fourth file. Nearest non-match, ruled out: "A --plugin-dir trial that tells the agent to hunt for a skill's own file breaks under a plugin name collision" — that entry governs how the dispatch prompt routes the agent, not what the trial surface exposes.*
...
Run 1 of the behavioral-claim trial ended with the judge returning `A: YES` — which, under the pre-registered decision rule, meant ship. The sentence the judge quoted as its evidence addressed the `Alternatives Considered` heading count; the criterion named the keyword-probe inference. Reading the quoted sentence against the criterion's own words was the only thing that caught it — the verdict line alone read as a clean pass, and a judge prompt that asked for bare YES/NO would have left nothing to check. Scored strictly, no arm had met its criterion, and the run was recorded as inconclusive rather than shipped. **Rule:** require the judge to quote the sentence it scored, and check that quote against the criterion's own wording before acting on any verdict — a positive result needs the same scenario-and-evidence scrutiny that Rule 3 already demands for a negative one, because a mis-scored YES ships a mechanism on manufactured evidence, which reads as validated forever after.

**Tags:** none yet — tags deferred.

*Pattern promoted — folded into docs/patterns/ab-test-live-trials-for-behavior-change.md as a fourth rule. Compared against all 28 existing Rule sentences per the promotion step's enumeration: one names the same act — "A negative trial result needs its scenario checked before it counts as evidence," which gathers a trial verdict and concludes a property of the mechanism without checking whether the verdict could mean what it appears to mean. Same act, opposite polarity; that entry is already Rule 3 of the same pattern, so this joins it as Rule 4 rather than opening a new file. Nearest non-match, ruled out: "A live trial priming a false belief needs a true A/B control" — that entry governs trial construction before the run, not how a returned verdict gets read.*

### An enumeration bounded by a line range or an expectation undercounts silently (2026-09-02-process-review-batch-r1-r4)

Designing a mirror of SDD's Finish list, the spec's author enumerated that section three times and undercounted it twice — the first read (a line range) missed concept-index, and the second, "complete" read missed three further members because it stopped where the previous read had. The same batch's own Finish then extracted Rule sentences with a line-anchored grep and captured 23 of 30; the file's own `**Rule:**` count exposed the gap, and seven rules turned out to wrap before their first period. Two earlier trial arms had already logged the same shape (17/25 and 16/17). Partial enumerations look identical to complete ones, so nothing downstream flags them. **Rule:** bound every enumeration by the structure's own terminator (the section's end, the file's end, the list's final delimiter) and reconcile an independent mechanical count against the enumeration's count before using the result — a read bounded by a line range or by the members you expected is not an enumeration.

**Tags:** none yet — tags deferred.

*Pattern promoted — see docs/patterns/enumerate-lists-to-their-structural-end.md. Compared against all 30 existing Rule sentences per the promotion step's enumeration (paragraph-joined extraction, count reconciled 30/30): none names this act. Nearest non-matches, ruled out: "a source document already read once needs a fresh full read immediately before building a second artifact from it" gathers a stale recall and concludes re-read for currency, where this entry gathers an in-progress partial read and concludes bound-by-structure plus count reconciliation — currency versus completeness; and "verifying a plan's numeric or pattern-matching claims means running the exact command against the exact real content" governs verifying a stated claim, not producing a complete member set. Promotes on both tests at once: prospective breadth (any every-member task) and recurrence (five logged instances).*
