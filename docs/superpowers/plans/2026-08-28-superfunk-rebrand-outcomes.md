...
Shipped as planned; added the fork-attribution blockquote right after the title, the Requirements section before Installation, and updated all 11 harness sections. Diverged from the plan's literal text in three sections (Codex App, Codex CLI, Cursor) exactly as the plan itself anticipated and explained: dropped the official-marketplace pointers (which only distribute upstream) in favor of direct-repository installs, matching the Antigravity/Pi pattern — not an unplanned deviation, the plan's own Step 5/6 called for this. Verified no stray `obra/superpowers` reference remains outside two correct, in-scope exceptions: the attribution note's own upstream link, and the `## Community` section's "Issues" link, which correctly still points at upstream Jesse Vincent/Prime Radiant's own issue tracker (out of scope per the spec — that section describes upstream's own community, not an install instruction). No unplanned divergence.
