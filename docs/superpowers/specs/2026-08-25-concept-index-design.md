...
2. A disposable `--plugin-dir` trial runs a 1-task plan that creates a new skill directory through `subagent-driven-development`. Finish adds exactly one new row for it, in its own separate commit — a stricter rule than the tracker/lessons-learned pattern, which sometimes shares one commit and sometimes doesn't; the index change always gets its own.
3. A second trial runs a 1-task plan that only modifies an existing skill's file, creating no new skill, feature, or significant directory. Finish correctly makes no change to the index. **Caveat:** the trial as run named the exact trigger paragraph and told the agent directly that nothing crossed a boundary, so it shows the instruction works when followed, not that a controller working the trigger condition unprompted would correctly discriminate a real non-crossing from a crossing. Treat this criterion as confirming the mechanism doesn't fire when told not to, not as confirming the trigger logic exercised itself end to end unprompted.
...
Colocating in `docs/architecture/` creates an unexamined interaction with `project-definition`'s own Step 1, which branches on whether that directory exists at all, not on its own specific output file. A codebase where `concept-index` runs before `project-definition` gets `docs/architecture/` created early, so `project-definition` finds it present and routes into "update existing sections" on a project with no project-definition document yet. `concept-index` itself has no equivalent bug (its own Step 1 checks the specific file). Fixing `project-definition`'s Step 1 to branch on `docs/architecture/project-definition.md` or `docs/architecture/NN-*.md` instead of the bare directory belongs on the follow-up list — tracked here, deliberately out of scope for this spec's own diff.
