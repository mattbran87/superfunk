...
3. A second trial runs a 1-task plan that only modifies an existing skill's file, creating no new skill, feature, or significant directory. Finish correctly makes no change to the index. **Caveat:** the trial as run named the exact trigger paragraph and told the agent directly that nothing crossed a boundary, so it shows the instruction is followable, not that a controller working the trigger condition unprompted would correctly discriminate a real non-crossing from a crossing. Treat this criterion as confirming the mechanism doesn't fire when told not to, not as confirming the trigger logic itself was exercised end to end.
