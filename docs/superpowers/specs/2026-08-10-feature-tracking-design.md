...
- **Multi-file scaling split (documented trigger and shape, not yet built)** — a module's `roadmap.md` passing roughly 150 lines signals a split. Past that point, `roadmap.md` turns into an index file: the H1, the generated Status Summary (aggregated across every bundle file instead of one file's own links), and a new Bundles index table (`Bundle | Features | Status | File`) linking to each bundle's detail file. Each bundle moves into its own `specs/<module>/roadmap-<bundle-slug>.md`, holding that bundle's heading and feature links — the same shape the body of `roadmap.md` holds today, just split apart. This trigger and target shape scale down `claude-spec-framework`'s own real split: spec `115-roadmap-multifile-decomposition` split a 2,213-line flat, whole-project roadmap into an index plus per-group and per-category files once scanning it stopped working. `superfunk` already avoids that failure mode's root cause for one dimension — a module boundary already separates roadmaps — so 150 lines gives enough room for a module to grow before the same problem repeats at a smaller scale. No code changes accompany this decision: `rebuild_index.py`'s status-summary pass still reads one file's own Bundle/link structure; extending it to aggregate across multiple bundle files waits for a real module to cross the threshold. This mirrors `claude-spec-framework`'s own deferral of its template update (decision D7) until the pattern proved out in practice — no module in `specs/` sits anywhere near 150 lines today.
...
- **Resolved 2026-08-11:** the multi-file scaling split, the fifth and final question from that comparison. `claude-spec-framework`'s real history — spec `115-roadmap-multifile-decomposition`, which split a 2,213-line flat roadmap into an index plus per-group and per-category files once it grew unwieldy — gave a proven pattern to scale down rather than design from scratch. `superfunk` now documents the trigger (~150 lines per module's `roadmap.md`) and the target shape (an index file with a Bundles table, one `roadmap-<bundle-slug>.md` per bundle) above, but doesn't build the split logic, since no module in `specs/` approaches the threshold. This matches `claude-spec-framework`'s own deferral of its template guidance (decision D7) until the pattern proved out in practice. All five items from the ranked Casita-comparison list now carry a resolution.
