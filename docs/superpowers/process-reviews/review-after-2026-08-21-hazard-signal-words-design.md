...
- [ ] Add a check to `plugin/skills/subagent-driven-development/task-reviewer-prompt.md`: before reporting a finding that claims a diff misrepresents or drifts from a cited source rule, re-read that rule's actual text directly and confirm the claim holds. This operationalizes `docs/patterns/verify-against-precedent-before-flagging.md` as a real pre-finding check, not only a recorded Lesson.
- [ ] Extend `plugin/skills/writing-plans/SKILL.md`'s Self-Review item 6 (Cross-file rule restatement) to also cover a single bullet that summarizes one source rule, not only restatements across multiple files — Task 5 and Task 6 of hazard-signal-words each show the narrower single-bullet case item 6's current wording doesn't quite name.
