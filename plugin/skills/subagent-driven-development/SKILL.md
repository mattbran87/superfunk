...
- **Pseudocode context:** determine which triggers (T1-T4) this task
...
`<plan-slug>` is the plan's filename with both the `.md` extension and
the leading `YYYY-MM-DD-` date prefix removed (`2026-08-25-concept-index.md`
→ `concept-index`) — the short form every real entry in that file
uses. The parenthetical plan-slug is what lets `process-review` group
Catches by spec later — a bare `Task <N>` is ambiguous across
sub-projects that each have their own Task 1. The review loop is
already doing the work; logging it costs one line and feeds
`process-review` later. A fix round is one fix dispatch plus one
scoped re-review. Five rounds
maximum per task:
...
same value `scripts/sdd-workspace` derives; the header below instead
uses `<plan filename>`, PLAN_FILE's filename unchanged, `.md` and
all). If this is the plan's first completed task, create the file
first with this header:
...
git commit -m "docs(plans): record Task <N> outcome for <plan-basename>"
```
...
| "The reviewer will just find something new anyway" | Scoped re-reviews verify fixes; they cannot wander. New findings on untouched code go to the ledger, not the loop — except a cross-section contradiction the fix itself caused, which extends the loop as New Breakage. |
...
Implementer: [Later]
  ...
  - Outcome: Shipped as planned; no divergence, no follow-ups.
  - Committed
...
[Append + commit outcomes entry for Task 1: docs/superpowers/plans/feature-plan-outcomes.md created]
...
[Append + commit outcomes entry for Task 2: docs/superpowers/plans/feature-plan-outcomes.md updated with the fix round's final Outcome]
