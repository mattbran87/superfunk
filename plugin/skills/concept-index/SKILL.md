...
A user may edit `docs/architecture/concept-index.md` directly (correcting a description, reordering rows). Never overwrite a hand-edited row without confirming first — the same living-document discipline `project-definition` applies to its own generated sections. Step 2 never faces this case: it only runs when no index exists yet, so there are no existing rows for it to overwrite. Step 3 is the only path that can overwrite or remove an existing row, and Step 3 only ever runs as part of an automated Finish-step trigger, never as a standalone human invocation — so its hand-edit protection is always the skip-and-flag mechanism specified in Step 3's item 2, not an interactive prompt.
