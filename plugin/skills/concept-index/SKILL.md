...
A user may edit `docs/architecture/concept-index.md` directly (correcting a description, reordering rows). Never overwrite a hand-edited row without flagging it first — the same living-document discipline `project-definition` applies to its own generated sections. This protection does not cover a hand-edited row order: Step 3's mismatch check only compares Description text, so a reordered-but-otherwise-unchanged row proceeds through Step 3 unnoticed. Step 2 never faces the overwrite case: it only runs when no index exists yet, so there are no existing rows for it to overwrite. Step 3 is the only path that can overwrite or remove an existing row. Step 3 only ever runs as part of an automated Finish-step trigger, never as a standalone human invocation. Its hand-edit protection is therefore always Step 3 item 2's skip-and-flag mechanism, never an interactive prompt.
