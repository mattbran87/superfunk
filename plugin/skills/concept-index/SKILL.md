...
2. Before updating or removing an existing row (items 3 and 4 below), re-read the unit's current underlying source (its `SKILL.md` frontmatter, `spec.md` heading, or `.context.md` Purpose line) and compare it word-for-word against the row's stored Description. A mismatch is not proof of a hand edit — Step 2's Skill-description trim is a judgment call, so two derivations can legitimately differ — so do not silently overwrite on a mismatch, and do not silently overwrite on a match either. Always flag the row in the Finish-step report, quoting both the stored and freshly-derived text, and let a human decide whether it was hand-edited before anything overwrites or removes it. See Updating an Existing Index by Hand, below, for the general policy this enforces. This check does not detect a hand-edited row order — reordering is not otherwise protected.
...
A user may edit `docs/architecture/concept-index.md` directly (correcting a description, reordering rows). Never overwrite a hand-edited row without flagging it first — the same living-document discipline `project-definition` applies to its own generated sections. Step 2 never faces this case: it only runs when no index exists yet, so there are no existing rows for it to overwrite. Step 3 is the only path that can overwrite or remove an existing row. Step 3 only ever runs as part of an automated Finish-step trigger, never as a standalone human invocation. Its hand-edit protection is therefore always the skip-and-flag mechanism specified in Step 3's item 2, never an interactive prompt.
