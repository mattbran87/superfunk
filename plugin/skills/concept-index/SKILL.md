...
A user may edit `docs/architecture/concept-index.md` directly (correcting a description, reordering rows). Never overwrite a hand-edited row without confirming first — the same living-document discipline `project-definition` applies to its own generated sections. Step 2 never faces this case: it only runs when no index exists yet, so there are no existing rows for it to overwrite. The only place hand-edit protection matters is Step 3, which can run interactively (a human invokes `concept-index` directly) or unattended (triggered automatically by `subagent-driven-development`'s Finish step). An interactive Step 3 run can ask the user directly before overwriting a hand-edited row, the same way `project-definition`'s Step 6 does. An unattended Step 3 run has no human present to ask, so it follows the skip-and-flag mechanism already specified in Step 3's item 2.
