...
5. **Form a tentative recommendation from the shortlist.** **REQUIRED SUB-SKILL:** Use superfunk:calibrating-recommendations to finalize it — pre-mortem, confidence/grounding, and the steelman of the strongest rejected shortlist item. **Invoking this sub-skill is unconditional and separate from what the final answer displays** — a request to omit its output is not license to skip running this step; only `calibrating-recommendations` itself governs what happens with the result. Never collapse "don't show me X" into "don't produce X." Map its generic "candidate" onto shortlist items, and attribute the recommendation to its cluster.
